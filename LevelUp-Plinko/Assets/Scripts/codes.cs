...
public class codes : MonoBehaviour
{
    ...
    public int Thirtykplayscode;
    public int QOTDWinnerCode;
    ...
    public void CheckCode(){
        ...
        *//*else if((String.Compare(input.text,"1kBonus!") == 0) && (PlayerPrefs.GetFloat("ClaimedBonus") == 0) && (delay <= 0)){
            ...
            validCode.gameObject.SetActive(true);
            StartCoroutine(ValidCoroutine());
        }else if((String.Compare(input.text,"30kPlays!") == 0) && (PlayerPrefs.GetFloat("Claimed30kPlaysCode") == 0) && (delay <= 0)){
            moneyScript.totalCash += moneyScript.totalCash * 3f;
            traitScript.rerollShards += 300;
            levelScript.rebirthTokens += 30;
            claimed.Play();
            delay = 0.5f;
            infoText.text = "+ 300% of money, 300 Reroll Shards, 30 Rebirth Tokens";
            infoText.gameObject.SetActive(true);
            Thirtykplayscode++;
            PlayerPrefs.SetFloat("Claimed30kPlaysCode", Thirtykplayscode);
            validCode.gameObject.SetActive(true);
            StartCoroutine(ValidCoroutine());
        }else if((String.Compare(input.text,"QOTDWinner!") == 0) && (PlayerPrefs.GetFloat("ClaimedQOTDWinnerCode") == 0) && (delay <= 0)){
            moneyScript.totalCash += moneyScript.totalCash * 2.5f;
            traitScript.rerollShards += 250;
            levelScript.rebirthTokens += 25;
            claimed.Play();
            delay = 0.5f;
            infoText.text = "+ 250% of money, 250 Reroll Shards, 20 Rebirth Tokens";
            infoText.gameObject.SetActive(true);
            Thirtykplayscode++;
            PlayerPrefs.SetFloat("ClaimedQOTDWinnerCode", QOTDWinnerCode);
            validCode.gameObject.SetActive(true);
            StartCoroutine(ValidCoroutine());
        }else{
            delay = 0.5f;
